function cell_to_index_map = register_cells(file_names, microns_per_pixel, model_type, save_directory)
%% Based CellReg's demo_2P
...
% Define the parameters for image alignment:
...
% Defining the parameters for the probabilistic modeling:
...
if save_figs 
    figures_directory = fullfile(save_directory, 'Figures');
...
if memory_efficient_run
    temp_dir = [save_directory, filesep, 'temp']; 
...
plot_all_sessions_projections(footprints_projections,save_directory,figures_visibility)
disp('Done')
...
if strcmp(alignment_type,'Non-rigid')
    plot_alignment_results(adjusted_spatial_footprints,centroid_locations,spatial_footprints_corrected,centroid_locations_corrected,adjusted_footprints_projections,footprints_projections_corrected,reference_session_index,all_projections_correlations,maximal_cross_correlation,alignment_translations,overlapping_FOV,alignment_type,number_of_cells_per_session,save_directory,figures_visibility,displacement_fields)
else
    plot_alignment_results(adjusted_spatial_footprints,centroid_locations,spatial_footprints_corrected,centroid_locations_corrected,adjusted_footprints_projections,footprints_projections_corrected,reference_session_index,all_projections_correlations,maximal_cross_correlation,alignment_translations,overlapping_FOV,alignment_type,number_of_cells_per_session,save_directory,figures_visibility)
end
...
plot_x_y_displacements(neighbors_x_displacements,neighbors_y_displacements,...
    ...
    save_directory,figures_visibility);
disp('Part a done')
...
plot_models(centroid_distances_model_parameters,NN_centroid_distances,...
    ...
    centers_of_bins,microns_per_pixel,normalized_maximal_distance,save_directory,...
...
centers_of_bins,normalized_maximal_distance,microns_per_pixel,save_directory,...
...
if ~isempty(model_type)
    initial_registration_type = model_type;
else
...
if strcmp(initial_registration_type,'Spatial correlation') % if spatial correlations are used
    disp("Using Spatial correlation model")
    if exist('spatial_correlation_intersection','var')
        ...
        plot_initial_registration(cell_to_index_map,number_of_bins,spatial_footprints_corrected,initial_registration_type,save_directory,figures_visibility,registered_cells_spatial_correlations,non_registered_cells_spatial_correlations)
else % if centroid distances are used
    disp("Using Centroid distance model")
    ...
    plot_initial_registration(cell_to_index_map,number_of_bins,spatial_footprints_corrected,initial_registration_type,save_directory,figures_visibility,registered_cells_centroid_distances,non_registered_cells_centroid_distances,microns_per_pixel,normalized_maximal_distance)
end
...
if strcmp(registration_approach,'Probabilistic')    
    ...
    plot_cell_scores(cell_scores_positive,cell_scores_negative,cell_scores_exclusive,cell_scores,p_same_registered_pairs,save_directory,figures_visibility)
...
if memory_efficient_run
    for file_n = 1:length(spatial_footprints_corrected)
        ...
        spatial_footprints_corrected{file_n} = fullfile(save_directory,f_name);
        save(fullfile(save_directory,f_name),'footprint');
    end
end
...
if save_results
    ...
    save(fullfile(save_directory,['cellRegistered_' datestr(clock,'yyyymmdd_HHMMss') '.mat']),'cell_registered_struct','-v7.3')
else
    if strcmp(registration_approach,'Probabilistic')
        cell_registered_struct.cell_scores=cell_scores';
        cell_registered_struct.true_positive_scores=cell_scores_positive';
...
if strcmp(registration_approach,'Probabilistic')
    if strcmp(model_type,'Spatial correlation')
        save_log_file(save_directory,file_names,microns_per_pixel,adjusted_x_size,adjusted_y_size,alignment_type, ...
            reference_session_index,maximal_distance,number_of_bins,initial_registration_type,initial_threshold, ...
            registration_approach,model_type,final_threshold,optimal_cell_to_index_map,cell_registered_struct,comments, ...
            uncertain_fraction_spatial_correlations,false_positive_per_correlation_threshold,true_positive_per_correlation_threshold,MSE_spatial_correlations_model)
    elseif strcmp(model_type,'Centroid distance')
        save_log_file(save_directory,file_names,microns_per_pixel,adjusted_x_size,adjusted_y_size,alignment_type, ...
            reference_session_index,maximal_distance,number_of_bins,initial_registration_type,initial_threshold, ...
            registration_approach,model_type,final_threshold,optimal_cell_to_index_map,cell_registered_struct,comments, ...
            uncertain_fraction_centroid_distances,false_positive_per_distance_threshold,true_positive_per_distance_threshold,MSE_centroid_distances_model)
    end
elseif strcmp(registration_approach,'Simple threshold')
    if strcmp(model_type,'Spatial correlation')
        save_log_file(save_directory,file_names,microns_per_pixel,adjusted_x_size,adjusted_y_size,alignment_type, ...
